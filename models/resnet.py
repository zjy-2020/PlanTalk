...
class ResConv2DBlock(nn.Module):
    def __init__(self, n_in, n_state, dilation=1, activation='silu', norm=None, dropout=0.2):
        ...
        self.conv1 = nn.Conv2d(n_in, n_state, 3, 1, padding, dilation)
        self.conv2 = nn.Conv2d(n_state, n_in, 1, 1, 0, )
        self.dropout = nn.Dropout(dropout)
...
class Resnet2D(nn.Module):
    def __init__(self, n_in, n_depth, dilation_growth_rate=1, reverse_dilation=True, activation='relu', norm=None):
        ...
        blocks = [ResConv2DBlock(n_in, n_in, dilation=dilation_growth_rate ** depth, activation=activation, norm=norm)
                  for depth in range(n_depth)]
        if reverse_dilation:
            blocks = blocks[::-1]
