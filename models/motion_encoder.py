...
from .utils.layer import ResBlock, ResBlock_2D, init_weight
...
class VQEncoderV6(nn.Module):
    ...
    def forward(self, inputs):
        inputs = inputs.permute(0, 2, 1)
        outputs = self.main(inputs).permute(0, 2, 1)
        # print('VQEncoderV6:',outputs.shape)
        return outputs
    
class VQEncoderV6_2D(nn.Module):

    def __init__(self, args):
        super(VQEncoderV6_2D, self).__init__()
        n_down = args.vae_layer
        channels = [args.vae_length]
        for i in range(n_down - 1):
            channels.append(args.vae_length)
        ...
        assert len(channels) == n_down
        layers = [
            nn.Conv2d(input_size, channels[0], kernel_size=(3, 3), stride=1, padding=(1, 1)),  # 2D 卷积
            nn.LeakyReLU(0.2, inplace=False),
            ResBlock_2D(channels[0]),
        ]

        for i in range(1, n_down):
            layers += [
                nn.Conv2d(channels[i - 1], channels[i], kernel_size=(3, 3), stride=1, padding=(1, 1)),  # 2D 卷积
                nn.LeakyReLU(0.2, inplace=False),
                ResBlock_2D(channels[i]),
            ]
        self.main = nn.Sequential(*layers)
        self.main.apply(init_weight)

    def forward(self, inputs):
        # inputs shape: (bs, j, t, c), we need to permute it to (bs, c, j, t)
        inputs = inputs.permute(0, 3, 1, 2)  # Shape becomes (bs, c, j, t)
        outputs = self.main(inputs)  # Apply 2D convolutions
        # The output should have shape (bs, vae_length, j, t), so permute back
        return outputs.permute(0, 2, 3, 1)  # Shape becomes (bs, j, t, vae_length)
