...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, ker_size, stride=1, downsample=False, cardinality=1, base_width=64,
        ...
        else: self.downsample=downsample
        self.stride = stride
    ...
    def forward(self, x):
        # 改动：移除势能共享下来的 shortcut
        original_x = x.clone()  # 保存原始输入的一个副本，用于下采样
        ...
        if self.downsample:
            shortcut_a = self.downsample(original_x)  # 使用 original_x 用于下采样
            x_6 = x_5 + shortcut_a
        else:
            x_6 = x_5 + x  # 直接使用 x，而不是 original_x
...
class ResBlock_2D(nn.Module): 
    def __init__(self, channel):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(channel, channel, kernel_size=(3, 3), stride=1, padding=(1, 1)),  # 2D 卷积
            nn.LeakyReLU(0.2, inplace=False),
            nn.Conv2d(channel, channel, kernel_size=(3, 3), stride=1, padding=(1, 1)),  # 2D 卷积
        )
