...
def generate_images(frames, vertices_all, vertices1_all, faces, output_dir,
    ...
    for i in range(frames):
        process_frame(i * 3, vertices_all, vertices1_all, faces, output_dir,
                      filenames)
...
def render_one_sequence(res_npz_path,
    ...
    time_s = time.time()
    generate_images(int(seconds * 10), vertices_all, vertices1_all, faces,
                    output_dir, filenames)
    filenames = [
        "{}frame_{}.png".format(output_dir, i * 3)
    ...
    imageio.mimsave(f"{output_dir}raw_{res_npz_path.split('/')[-1][:-4]}.mp4",
                    images,
                    fps=10)
    ...
    video = mp.VideoFileClip(
        f"{output_dir}raw_{res_npz_path.split('/')[-1][:-4]}.mp4")
    ...
    final_clip.write_videofile(
        f"{output_dir}{res_npz_path.split('/')[-1][4:-4]}.mp4")
